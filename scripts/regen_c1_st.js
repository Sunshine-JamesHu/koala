...
const sbPath = path.join(baseDir, '03_storyboard.md');
...
out.push("> **用途**: 短视频/推文解说配音脚本 (完全 1:1 对齐 40 个分镜)");
out.push("> **配音基调**: 网感、吐槽、悬疑跌宕、绝地求生");
out.push("---");

for (let i = 0; i < shots.length; i++) {
    const s = shots[i];
    out.push(`## 镜${i + 1} (\`C01_${s.id}.jpg\`)`);
    out.push("");
    out.push("**画面描述**：");
    out.push(`主体与动作：${s.desc}`);
    out.push(`构图与光影：${s.camera}。`);

    if (i < 15) {
        out.push("情绪氛围：极高对比度的昏暗压迫感与死寂。");
    } else if (i < 28) {
        out.push("情绪氛围：命悬一线的极致动态冲刺感与寒意。");
    } else if (i < 35) {
        out.push("情绪氛围：力量碾压与绝望感。");
    } else {
        out.push("情绪氛围：戏精附体的高能对峙与诡异嘲弄。");
    }
    out.push("");
    out.push("**旁白文案**：");
...
console.log(`SUCCESS: Generated ${shots.length} shots for 1:1 match in 05_storytelling.md`);
