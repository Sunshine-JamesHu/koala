...
**此镜头无需生成首帧图**，直接使用 `shot_016_last_frame.png` 作为首帧钉定图。
...
2. 上传参考图（按顺序）：
   - shot_016_last_frame.png（上一镜头尾帧，确保衔接）
   - 沈清起-表情卡.png
...
1. 打开 **Nano Banana Pro**
2. 上传参考图（按顺序）：
   - shot_016_last_frame.png（作为首帧参考）
   - 辛月影-表情卡.png
   - 沈清起-表情卡.png
...
3. 选择时长：**8秒**
4. 上传图片（按顺序，最多5张）：
   - shot_016_last_frame.png（首帧钉定，复用上一镜头尾帧）
...
- [ ] 确认已复用 shot_016_last_frame.png 作为首帧
- [ ] 尾帧图已生成并保存
- [ ] 9分镜组合图已生成并保存
- [ ] 视频已生成并保存
