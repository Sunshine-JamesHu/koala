...
**后续章节（第2章及之后）的场景图必须基于前一章已生成的场景图进行延展，而不是每次都从头生成！**

### 为什么这很重要？

| 方式 | 结果 | 问题 |
|------|------|------|
| ❌ 每章都从头生成场景图 | 场景细节每章都略有不同 | 道具位置"穿帮"，光影不一致 |
| ✅ 基于前一章场景图延展 | 场景细节完全一致 | 保持连贯性，观看体验流畅 |
...
| 章节 | 场景策略 | 说明 |
|------|----------|------|
| **第1章** | 从场景参考生成 | 建立场景的视觉基础 |
| **第2章及之后** | **基于前一章场景图延展** | 确保场景细节完全一致 |

### 场景变化类型与处理方式

| 变化类型 | 处理方式 | 参考图 |
|----------|----------|--------|
| **无变化** | 直接复用前一章场景图 | 前一章场景图 |
| **时间变化** | 基于前一章场景图修改光影 | 前一章场景图 + 光影描述 |
| **道具变化** | 基于前一章场景图增删道具 | 前一章场景图 + 道具描述 |
| **破坏/损坏** | 基于前一章场景图添加破坏效果 | 前一章场景图 + 破坏描述 |
| **全新场景** | 从场景参考图生成 | 场景参考/大全景 |
...
#### 第2章场景生成（延展流程）⭐
```markdown
#### BG-01B: 草屋内 - 深夜（延展自第1章）

**场景状态**: 基于第1章延展

**与前一章的差异**: 时间从黄昏变为深夜，光线变暗

**需要上传的图片**:
| 顺序 | 图片名称 | 用途 |
|------|----------|------|
| 参考图1 | chapter_001/BG-01_草屋内-大全景 | ⭐前一章场景图（确保空间一致） |

**提示词**:
\```
[STYLE_PREFIX],

Based on the previous scene (chapter_001 BG-01),
same room layout and furniture positions,
[变化描述: later at night, darker atmosphere, single candle light],
maintaining the same spatial relationships and perspective,
...
**场景延展说明**:
- 本场景 [是/否] 基于前一章场景延展
- 参考图来源: chapter_XXX / 场景参考（首次建立）
- 变化描述: [说明与前一场景的差异，如无变化则写"无变化"]
```
...
| 顺序 | 图片名称 | 用途 |
|------|----------|------|
| 参考图1 | 草屋内-布局参考 | 场景布局参考 |  ← ❌ 错误！应该用第1/2章的场景图
```
...
**场景延展说明**:
- 本场景基于 chapter_002/BG-XX 延展
- 变化描述: 油灯刚点亮的瞬间，光影从暗到明的变化

**需要上传的图片**:
| 顺序 | 图片名称 | 用途 |
|------|----------|------|
| 参考图1 | chapter_002/BG-XX_草屋内-深夜 | ⭐前一章场景图（确保空间一致） |
```
